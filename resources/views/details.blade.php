                    ...
                    @if(Auth::user()->matricnum !== $product->matricnum)
                        ...
                        @else
                            <div class="flex flex-wrap gap-4 pt-4">
                                ...
                                <a href="{{ route('chat.seller', ['seller_id' => $product->user->matricnum]) }}" class="bg-[#E95670] text-white px-6 py-3 rounded-lg text-lg font-semibold hover:bg-[#B34270] transition">
                                    Chat Seller
                                </a>

                                <!-- Book Item Button -->
                                @if($product->stock_status !== 'sold' && Auth::user()->matricnum !== $product->matricnum)
                                    <form action="{{ route('booking.store', $product->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="bg-blue-500 text-white px-6 py-3 rounded-lg text-lg font-semibold hover:bg-blue-400 transition">
                                            Book Item
                                        </button>
                                    </form>
                                @endif
